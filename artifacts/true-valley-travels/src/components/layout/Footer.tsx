          ...
          <div className="lg:col-span-1">
            ...
            <div className="flex gap-3">
              {[
                ...
                <motion.a key={i} href="#" className="w-9 h-9 bg-white/10 hover:bg-secondary flex items-center justify-center transition-colors" whileHover={{ scale: 1.12, y: -2 }}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d={d} /></svg>
                </motion.a>
              ))}
            </div>
          </div>
          ...
          {Object.entries(cols).map(([heading, links]) => (
            <div key={heading}>
              ...
              <ul className="space-y-3">
                {links.map((l) => (
                  <li key={l.label}>
                    <motion.a href={l.href} className="text-sm hover:text-secondary transition-colors flex items-center gap-2 group" whileHover={{ x: 5 }}>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm">
              {[
                ...
                { icon: "M22 16.92v3a2 2 0 0 1-2.18 2A19.79 19.79 0 0 1 11.69 19a19.5 19.5 0 0 1-6-6A19.79 19.79 0 0 1 2.12 6.18 2 2 0 0 1 4.11 4h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 11.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z", text: "+91 98765 43210" },
              ].map((item, i) => (
                <li key={i} className="flex items-start gap-3">
                  <svg className="w-4 h-4 text-secondary shrink-0 mt-0.5" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d={item.icon} /></svg>
                  {item.text}
                </li>
              ))}
            </ul>
